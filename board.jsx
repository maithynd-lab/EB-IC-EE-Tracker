...
const LS = { tasks: 'ttb_tasks_v2', tags: 'ttb_tags_v3', sort: 'ttb_sort_v2', posts: 'ttb_posts_v2', channels: 'ttb_channels_v1', scope: 'ttb_scope_v1', commNotes: 'ttb_commnotes_v1', projNotes: 'ttb_projnotes_v1' };
...
function App() {
  ...
  const [projNotes, setProjNotes] = useLocal(LS.projNotes, {});
  ...
  const range = view === 'week' ? weekRange(ref) : (view === 'month' || view === 'gantt') ? monthRange(ref) : null;
  ...
  const periodLabel = view === 'week' ? weekLabel(range) : (view === 'month' || view === 'gantt') ? monthLabel(ref) : '';
  ...
  const openNewForProject = (tagId) => {
    const owner = members[0].id;
    const max = Math.max(0, ...tasks.filter((x) => x.owner === owner).map((x) => x.order));
    setEditing({ id: uid(), owner, title: '', note: '', tagIds: tagId ? [tagId] : [], priority: null,
                 deadline: null, done: false, completedAt: null, phase: 'pre', order: max + 1, isNew: true });
  };
      ...
      <header className="topbar">
        ...
        <div className="toolbar">
          <div className="seg viewseg">
            {[['board', 'Board'], ['gantt', 'Gantt Chart']].map(([k, l]) => (
      ...
      {view === 'board' ? (
        ...
        <main className="report-main">
          <div className="subbar">
            ...
            <div className="subbar-title">{view === 'week' ? 'Báo cáo tuần · cho team' : view === 'gantt' ? 'Dashboard projects · Gantt theo tháng' : 'Báo cáo tháng · cho Sếp Hải'}</div>
          ...
          {view === 'week'
            ...
            : view === 'gantt'
            ? <ProjectGantt members={members} tasks={tasks} tags={tags} range={range} onOpen={openEdit}
                            monthLabel={monthLabel(ref)} onStepMonth={stepPeriod} onToday={() => setRef(todayISO())}
                            onNewProject={() => openNewEvent(todayISO())} onNewTask={openNewForProject}
                            onEditProject={openEditEvent} notes={projNotes} setNotes={setProjNotes} />
